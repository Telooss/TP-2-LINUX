...
```
[balan@storage ~]$ sudo pvcreate /dev/sdb
[sudo] password for balan:
  Physical volume "/dev/sdb" successfully created.
```

```
[balan@storage ~]$ sudo pvs
...
```
[balan@storage ~]$ sudo vgcreate storage /dev/sdb
  Volume group "storage" successfully created
```

```
[balan@storage ~]$ sudo vgs
  Devices file sys_wwid t10.ATA_____VBOX_HARDDISK___________________________VB89ec5d3b-5728a15a_ PVID DxP2Y1leKoh4lQlag4E8anxQtLwl1cCI last seen on /dev/sda2 not found.
...
```
[balan@storage ~]$ sudo lvcreate -l 100%FREE storage -n last_data
  Logical volume "last_data" created.
```

```
[balan@storage ~]$ sudo lvs
  Devices file sys_wwid t10.ATA_____VBOX_HARDDISK___________________________VB89ec5d3b-5728a15a_ PVID DxP2Y1leKoh4lQlag4E8anxQtLwl1cCI last seen on /dev/sda2 not found.
...
```
[balan@storage ~]$ sudo mkfs -t ext4 /dev/storage/last_data
...
```
[balan@storage ~]$ sudo mkdir /mnt/storage
```

```
[balan@storage ~]$ sudo mount /dev/storage/last_data /mnt/storage/
```

```
[balan@storage ~]$ df -h | grep /storage
/dev/mapper/storage-last_data  2.0G   24K  1.9G   1% /mnt/storage
```

```
[balan@storage ~]$ ls -la /dev/mapper/storage-last_data
...
```
[balan@storage ~]$ cat /etc/fstab | grep storage
/dev/storage/last_data /mnt/storage/ ext4 defaults 0 0
```

```
[balan@storage ~]$ sudo umount /mnt/storage/
```

```
[balan@storage ~]$ sudo mount -av
/                        : ignored
...
```
[balan@storage ~]$ sudo dnf install nfs-utils
```

```
[balan@storage ~]$ sudo mkdir /mnt/storage/site_web_1
[balan@storage ~]$ sudo mkdir /mnt/storage/site_web_2
```

```
[balan@storage ~]$ sudo systemctl enable nfs-server
[balan@storage ~]$ sudo systemctl start nfs-server
[balan@storage ~]$ sudo !!
sudo firewall-cmd --permanent --add-service=nfs
success
[balan@storage ~]$ sudo firewall-cmd --permanent --add-service=mountd
success
[balan@storage ~]$ sudo firewall-cmd --permanent --add-service=rpc-bind
success
[balan@storage ~]$ sudo firewall-cmd --reload
success
[balan@storage ~]$ sudo firewall-cmd --permanent --list-all | grep services
  services: cockpit dhcpv6-client mountd nfs rpc-bind ssh
```


```
[balan@storage ~]$ cat /etc/exports
...
```
[balan@web ~]$ sudo dnf install nfs-utils
```

```
[balan@web ~]$ sudo mkdir -p /var/www/site_web_1/
[balan@web ~]$ sudo mkdir -p /var/www/site_web_2/
[balan@web ~]$ sudo mount 192.168.1.3:/mnt/storage/site_web_1/ /var/www/site_web_1/
[balan@web ~]$ sudo mount 192.168.1.3:/mnt/storage/site_web_2/ /var/www/site_web_2/
```

```
[balan@web ~]$ df -h | grep storage
...
```
[balan@web ~]$ sudo dnf install nginx
```

## 3. Analyse

```
[balan@web ~]$ sudo systemctl start nginx
[balan@web ~]$ sudo systemctl status nginx
...
```
[balan@web ~]$ ps -ef | grep nginx
root        2171       1  0 14:06 ?        00:00:00 nginx: master process /usr/sbin/nginx
nginx       2172    2171  0 14:06 ?        00:00:00 nginx: worker process
balan       2183    1210  0 14:07 pts/0    00:00:00 grep --color=auto nginx
```
```
[balan@web ~]$ sudo ss -lntup | grep nginx
...
```
```
[balan@web ~]$ ls -al /usr/share/nginx/html/index.html 
...
```
[balan@web ~]$ sudo firewall-cmd --zone=public --permanent --add-service=http
success
[balan@web ~]$ sudo firewall-cmd --reload
success
```
...
```
alanw@TelosGVNG MINGW64 ~ $  curl http://192.168.1.2
<!doctype html>
<html>
  <head>
    <meta charset='utf-8'>
...
```
[balan@web ~]$ sudo cat /var/log/nginx/access.log | tail -n 3
...
```
[balan@web ~]$ sudo firewall-cmd --zone=public --permanent --remove-service=http
success
[balan@web ~]$ sudo firewall-cmd --zone=public --permanent --add-port 8080/tcp
success
[balan@web ~]$ sudo firewall-cmd --reload
success
```
...
```
alanw@TelosGVNG MINGW64 ~ $ curl http://192.168.1.2:8080
<!doctype html>
<html>
  <head>
    <meta charset='utf-8'>
...
```
[balan@web ~]$ sudo useradd web
[balan@web ~]$ sudo passwd web
Changing password for user web.
```
```
[balan@web ~]$ sudo cat /etc/nginx/nginx.conf | grep user
user web;
[balan@web ~]$ ps -ef | grep nginx
root        2587       1  0 14:35 ?        00:00:00 nginx: master process /usr/sbin/nginx
web         2588    2587  0 14:35 ?        00:00:00 nginx: worker process
balan       2596    1210  0 14:35 pts/0    00:00:00 grep --color=auto nginx
```
...
```
[balan@web ~]$ sudo cat /etc/nginx/nginx.conf | grep web
...
```
[balan@web ~]$ sudo cat /etc/nginx/nginx.conf | grep conf.d
...
```
[balan@web ~]$ cat /etc/nginx/conf.d/site_web_1.conf 
server {
	listen       8080;
        listen       [::]:8080;
        server_name  _;
...
```
[balan@web ~]$ cat /etc/nginx/conf.d/site_web_2.conf 
server {
        listen       8888;
        listen       [::]:8888;
        server_name  _;
...
```
[balan@web ~]$ sudo firewall-cmd --zone=public --permanent --add-port 8888/tcp
success
[balan@web ~]$ sudo systemctl restart nginx
[balan@web ~]$ sudo firewall-cmd --reload
success
```
...
alanw@TelosGVNG MINGW64 ~ $  curl http://192.168.1.2:8888
 <!DOCTYPE html>
<html>
<body>
